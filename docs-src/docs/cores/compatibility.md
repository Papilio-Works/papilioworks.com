---
id: compatibility
title: Core Compatibility List
sidebar_label: Compatibility List
sidebar_position: 5
---

# Core Compatibility List

Overview of all cores — current status, what works, and what's coming.

| Core | System | Year | Status | Notes |
|---|---|---|---|---|
| A2600Nano | Atari 2600 | 1977 | ✓ Working | Most cartridges. Bankswitching WIP |
| C64Nano | Commodore 64 | 1982 | ✓ Working | D64, PRG, CRT, TAP supported |
| SNESTang | Super Nintendo | 1990 | 🔧 In Progress | Mode 7 ✓, SuperFX pending |
| NESTang | NES | 1983 | 🔧 In Progress | Core mapper support WIP |
| GBATang | Game Boy / GBA | 1989/2001 | 🔍 Investigating | Feasibility under review |
| MiSTeryNano | Atari ST | 1985 | 🔍 Investigating | Depends on harbaum collaboration |
| NanoMig | Amiga | 1985 | 🔍 Investigating | Depends on harbaum collaboration |

---

:::note[Content Coming Soon]
Detailed per-game compatibility tables will be added here as testing data is collected. Community contributions welcome — see [Contributing Cores](../advanced/contributing-cores).
:::

---

## How to Follow Development

- GitHub: [https://github.com/Papilio-Retrocade](https://github.com/Papilio-Retrocade)
- Community: [https://community.papilioworks.com](https://community.papilioworks.com)

---

## 🎓 Want to Go Deeper?

Want to port a core to the Retrocade yourself?

**[Retro Game Core Development on Papilio Retrocade →](https://learn.papilioworks.com)**
